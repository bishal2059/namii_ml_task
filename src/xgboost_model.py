...
grid_params = {
    'feature_sel__select_k__k': [ 1000, all],                 
    'feature_sel__var_thresh__threshold': [1e-4, 1e-2],       
    'feature_sel__select_k__score_func': [f_classif],        
...
best_params_df.to_csv('../results/best_params_xgboost.csv', index=False)
print("Best hyperparameters saved to '../results/best_params_xgboost.csv'.")

# Save test metrics to CSV
metrics_df = pd.DataFrame(metrics_test, index=[0])
metrics_df.to_csv('../results/test_metrics_xgboost.csv', index=False)
print("Test metrics saved to '../results/test_metrics_xgboost.csv'.")
...
predictions_df.to_csv('../results/blinded_predictions_xgboost.csv', index=False)
print("Blinded predictions saved to '../results/blinded_predictions_xgboost.csv'.")
