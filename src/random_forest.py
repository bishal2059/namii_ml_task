...
# --- 3. Build & Fit Random Forest Model ---
pipeline = Pipeline([
...
grid_params = {
    'feature_sel__select_k__k': [500, 1000,1500,all],                 
    'feature_sel__var_thresh__threshold': [1e-4, 1e-2,1e-1,1],       
    'feature_sel__select_k__score_func': [f_classif,mutual_info_classif],       
...
best_params_df.to_csv('../results/best_params_random_forest.csv', index=False)
print("Best hyperparameters saved to '../results/best_params_random_forest.csv'.")

# Save test metrics to CSV
metrics_df = pd.DataFrame(metrics_test, index=[0])
metrics_df.to_csv('../results/test_metrics_random_forest.csv', index=False)
print("Test metrics saved to '../results/test_metrics_random_forest.csv'.")
...
predictions_df.to_csv('../results/blinded_predictions_random_forest.csv', index=False)
print("Blinded predictions saved to '../results/blinded_predictions_random_forest.csv'.")
